...
public class RenderUtils
{
    ...
    public static void drawRainbow(double x, double y, double x1, double y1, int col1, int col2, int col3, int col4) {
        ...
        GL11.glBegin(7);
        //??????? ???
        glColor4f((float) c1.getRed(), (float) c1.getGreen(), (float) c1.getBlue(), (float) c1.getAlpha());
        GL11.glVertex2d(x, y);
        glColor4f((float) c2.getRed(), (float) c2.getGreen(), (float) c2.getBlue(), (float) c2.getAlpha());
        GL11.glVertex2d(x, y1);
        glColor4f((float) c3.getRed(), (float) c3.getGreen(), (float) c3.getBlue(), (float) c3.getAlpha());
        GL11.glVertex2d(x1, y1);
        glColor4f((float) c4.getRed(), (float) c4.getGreen(), (float) c4.getBlue(), (float) c4.getAlpha());
...
//        GL11.glEnable(GL_MULTISAMPLE);
    ...
    public static void drawCustomImage(float v, float v1, int i, int i1, ResourceLocation resourceLocation, int i2) {
        ...
        GL11.glDepthMask(false);
        OpenGlHelper.glBlendFunc(770, 771, 1, 0);
        glColor4f(new Color(i2).getRed() / 255f, new Color(i2).getGreen() / 255f, new Color(i2).getBlue() / 255f, new Color(i2).getAlpha() / 255f);
...
//        GL11.glHint(3155, 4352);
    ...
    public static void drawCustomImageAlpha(float v, float v1, int i, int i1, ResourceLocation resourceLocation, int i2, float alpha) {
        ...
        GL11.glDepthMask(false);
        OpenGlHelper.glBlendFunc(770, 771, 1, 0);
        glColor4f(new Color(i2).getRed() / 255f, new Color(i2).getGreen() / 255f, new Color(i2).getBlue() / 255f, alpha / 255f);
        Minecraft.getMinecraft().getTextureManager().bindTexture(resourceLocation);
        Gui.drawModalRectWithCustomSizedTexture((int) v, (int) v1, 0.0f, 0.0f, i, i1, (float) i, (float) i1);
        GL11.glDepthMask(true);
        GL11.glDisable(3042);
    ...
    public static void scissor(double x, double y, double width, double height) {
        final ScaledResolution sr = new ScaledResolution(CobaltGraphics.mc);
    ...
    public static void draw2D(final Entity entityLiving, final float partialTicks, final float lineWidth, final Color color) {
        ...
        GL11.glPushMatrix();
        GL11.glLoadIdentity();
        GL11.glOrtho(0.0, CobaltGraphics.mc.displayWidth, CobaltGraphics.mc.displayHeight, 0.0, -1.0, 1.0);
        ...
        final RenderManager renderManager = CobaltGraphics.mc.getRenderManager();
        ...
        for (final double[] boxVertex : boxVertices) {
            final Vector2f screenPos = WorldToScreen.worldToScreen(new Vector3f((float)boxVertex[0], (float)boxVertex[1], (float)boxVertex[2]), mvMatrix, projectionMatrix, CobaltGraphics.mc.displayWidth, CobaltGraphics.mc.displayHeight);
            if (screenPos != null) {
        ...
        if (minX > 0.0f || minY > 0.0f || maxX <= CobaltGraphics.mc.displayWidth || maxY <= CobaltGraphics.mc.displayWidth) {
            glColor4f(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, 1.0f);
            GL11.glBegin(2);
    ...
    public static void drawTargetESP(final EntityLivingBase target, final Color color, final float partialTicks) {
        ...
        GlStateManager.translate(target.lastTickPosX + (target.posX - target.lastTickPosX) * partialTicks - CobaltGraphics.mc.getRenderManager().viewerPosX, target.lastTickPosY + (target.posY - target.lastTickPosY) * partialTicks - CobaltGraphics.mc.getRenderManager().viewerPosY + target.height * location, target.lastTickPosZ + (target.posZ - target.lastTickPosZ) * partialTicks - CobaltGraphics.mc.getRenderManager().viewerPosZ);
        enableGL2D();
        GL11.glShadeModel(7425);
        GL11.glDisable(2884);
        ...
        GL11.glBegin(5);
        for (int i = 0; i <= 120; ++i) {
            glColor4f(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, 0.5f);
            final double x = Math.cos(i * 3.141592653589793 / 60.0) * target.width;
            final double z = Math.sin(i * 3.141592653589793 / 60.0) * target.width;
            GL11.glVertex3d(x, 0.15000000596046448 * cos, z);
            glColor4f(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, 0.2f);
            GL11.glVertex3d(x, -0.15000000596046448 * cos, z);
        }
        GL11.glEnd();
        GL11.glShadeModel(7424);
    ...
    public static void drawRectWith2Colors(float left, float top, float right, float bottom, final int color, final int color2) {
        ...
        worldrenderer.pos(right, bottom, 0.0).color(f4, f5, f6, f3).endVertex();
        worldrenderer.pos(right, top, 0.0).color(f4, f5, f6, f3).endVertex();
        worldrenderer.pos(left, top, 0.0).color(ff4, ff5, ff6, ff3).endVertex();
        tessellator.draw();
    ...
    public static void drawImage(final ResourceLocation image, final float x, final float y, final float width, final float height, final float alpha) {
        ...
        CobaltGraphics.mc.getTextureManager().bindTexture(image);
    ...
    public static void renderEspBox(BlockPos blockPos, float partialTicks, int color, float opacity) {
        if (blockPos != null) {
            IBlockState blockState = CobaltGraphics.mc.theWorld.getBlockState(blockPos);

            if (blockState != null) {
                Block block = blockState.getBlock();
                block.setBlockBoundsBasedOnState(CobaltGraphics.mc.theWorld, blockPos);
                double d0 = CobaltGraphics.mc.thePlayer.lastTickPosX + (CobaltGraphics.mc.thePlayer.posX - CobaltGraphics.mc.thePlayer.lastTickPosX) * (double) partialTicks;
                double d1 = CobaltGraphics.mc.thePlayer.lastTickPosY + (CobaltGraphics.mc.thePlayer.posY - CobaltGraphics.mc.thePlayer.lastTickPosY) * (double) partialTicks;
                double d2 = CobaltGraphics.mc.thePlayer.lastTickPosZ + (CobaltGraphics.mc.thePlayer.posZ - CobaltGraphics.mc.thePlayer.lastTickPosZ) * (double) partialTicks;
                drawFilledBoundingBox(block.getSelectedBoundingBox(CobaltGraphics.mc.theWorld, blockPos).expand(0.002D, 0.002D, 0.002D).offset(-d0, -d1, -d2), color, opacity);
            }
        }
    }
    ...
    public static void blockBox(final TileEntity block, final Color color) {
        ...
        GL11.glDepthMask(false);
        setColor(color);
        RenderGlobal.drawSelectionBoundingBox(block.getRenderBoundingBox().offset(-CobaltGraphics.mc.getRenderManager().viewerPosX, -CobaltGraphics.mc.getRenderManager().viewerPosY, -CobaltGraphics.mc.getRenderManager().viewerPosZ));
        GL11.glEnable(3553);
        GL11.glEnable(2929);
        GL11.glDepthMask(true);
        GL11.glDisable(3042);
    ...
    public static void drawGradientOutlinedRoundedRect(float x, float y, final float width, final float height, final float radius, final float linewidth, final int color1, final int color2, final int color3, final int color4) {
        ...
        GL11.glShadeModel(7425);
        GL11.glBegin(2);
        glColor4f(red1, green1, blue1, opacity1);
        for (int i = 0; i <= 90; i += 3) {
        ...
        glColor4f(red2, green2, blue2, opacity2);
        for (int i = 90; i <= 180; i += 3) {
        ...
        glColor4f(red3, green3, blue3, opacity3);
        for (int i = 0; i <= 90; i += 3) {
            GL11.glVertex2d(x2 - radius + Math.sin(i * 3.141592653589793 / 180.0) * radius, y2 - radius + Math.cos(i * 3.141592653589793 / 180.0) * radius);
        }
        glColor4f(red4, green4, blue4, opacity4);
        for (int i = 90; i <= 180; i += 3) {
            GL11.glVertex2d(x2 - radius + Math.sin(i * 3.141592653589793 / 180.0) * radius, y + radius + Math.cos(i * 3.141592653589793 / 180.0) * radius);
        }
        GL11.glEnd();
    ...
    public static Vec3 getInterpolatedPos(final float partialTicks) {
        return new Vec3(interpolate(CobaltGraphics.mc.thePlayer.prevPosX, CobaltGraphics.mc.thePlayer.posX, partialTicks), interpolate(CobaltGraphics.mc.thePlayer.prevPosY, CobaltGraphics.mc.thePlayer.posY, partialTicks) + 0.1, interpolate(CobaltGraphics.mc.thePlayer.prevPosZ, CobaltGraphics.mc.thePlayer.posZ, partialTicks));
    }
    ...
    public static void renderStarredNametag(final Entity entityIn, final String str, final double x, final double y, final double z, final int maxDistance) {
        final double d0 = entityIn.getDistanceSqToEntity(CobaltGraphics.mc.getRenderManager().livingPlayer);
        if (d0 <= maxDistance * maxDistance) {
            final FontRenderer fontrenderer = CobaltGraphics.mc.getRenderManager().getFontRenderer();
            final float f = 1.6f;
            final float f2 = 0.016666668f * f;
            GlStateManager.pushMatrix();
            GlStateManager.translate((float)x + 0.0f, (float)y + entityIn.height + 0.5f, (float)z);
            GL11.glNormal3f(0.0f, 1.0f, 0.0f);
            GlStateManager.rotate(-CobaltGraphics.mc.getRenderManager().playerViewY, 0.0f, 1.0f, 0.0f);
            GlStateManager.rotate(CobaltGraphics.mc.getRenderManager().playerViewX, 1.0f, 0.0f, 0.0f);
    ...
    public static void renderLivingLabel(final Entity entityIn, final String str, final double x, final double y, final double z, final int maxDistance) {
        final double d0 = entityIn.getDistanceSqToEntity(CobaltGraphics.mc.thePlayer);
        if (d0 <= maxDistance * maxDistance) {
            final FontRenderer fontrenderer = CobaltGraphics.mc.getRenderManager().getFontRenderer();
            final float f = 1.6f;
            final float f2 = 0.016666668f * f;
